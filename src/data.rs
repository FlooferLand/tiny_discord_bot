...
pub async fn save_data<'a>(ctx: Context<'a>) -> Result<(), BotError> {
	for server in ctx.data().servers.iter() {
		save_server(ctx, &server).await?
	}
	Ok(())
}

pub async fn save_server<'a>(ctx: Context<'a>, server: &Server) -> Result<(), BotError> {
	let guild = ctx.http().get_guild(GuildId::new(server.id)).await;
	match serde_yml::to_string(&server.to_serde().await) {
		Ok(out) => {
			...
			// Writing the file
			let path = format!("./assets/data/servers/{}.yml", server.id);
			let _ = std::fs::create_dir_all("./assets/data/servers/");
